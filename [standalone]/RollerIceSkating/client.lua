...
local haveRoller = false

RegisterNetEvent("pata_roller:roller")
AddEventHandler("pata_roller:roller", function(id)
	if not haveRoller then
		haveRoller = true
...
RegisterNetEvent("pata_roller:iceroller")
AddEventHandler("pata_roller:iceroller", function(id)
	if not haveRoller then
	haveRoller = true
	CurrentShoes = GetPedDrawableVariation(PlayerPedId(), 6)
	CurrentShoesColor = GetPedTextureVariation(PlayerPedId(),6)
	...
	if getSex() then
		SetPedComponentVariation(PlayerPedId(),6,Config.ICERollerSlotHomme,Config.ICERollerColorHomme,2)
	else
		SetPedComponentVariation(PlayerPedId(),6,Config.ICERollerSlotFemme,Config.ICERollerColorFemme,2)
	end
	
	TriggerEvent("roller:attach","IceRoller")
...
RegisterNetEvent("pata_roller:leaveroller")
AddEventHandler("pata_roller:leaveroller", function(id)
	actif = false
	Citizen.Wait(1500)
	RCCar.AttachPlayer(false)
	-- Citizen.Wait(1500)
...
if UseCommands then
	...
	RegisterCommand("iceroller", function()
		-- print("command spawn
		if not haveRoller then
		haveRoller = true
		CurrentShoes = GetPedDrawableVariation(PlayerPedId(), 6)
...
AddEventHandler("roller:attach", function(id,typeOf)
	RCCar.AttachPlayer(true,typeOf)
end)
...
RCCar.AttachPlayer = function(toggle,typeOfRoller)
	if toggle then
		if typeOfRoller == "IceRoller" then
		AttachEntityToEntity(player, RCCar.Entity, 20, 0.0, 0, 0.60, 0.0, 0.0, 0.0, false, false, false, true, 1, true)
