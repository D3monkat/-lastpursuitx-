...
local activeActions = {} -- This saves Utils.eventsPrefix .. ':checkAllowedActions' data
...
local function checkAllowedActions()
    ...
    activeActions = TriggerServerPromise(Utils.eventsPrefix .. ':checkAllowedActions')
    
    for action, enabled in pairs(activeActions) do
        if(enabled and not ACTIONS_BLACKLIST[action]) then canDoAnyAction = true break end
    end

    actionsMenuEnabled = activeActions.actionsMenuEnabled
    placeableObjects = activeActions.placeableObjects
    ...
    local actions = {
        {condition = activeActions.enableBilling, label = getLocalizedText('actions_billing'), value = 'createBilling', type = 'player'},
        {condition = activeActions.canRob, label = getLocalizedText('actions_search'), value = 'search', type = 'player'},
        {condition = activeActions.canHandcuff, label = getLocalizedText('actions_put_soft_handcuffs'), value = 'softHandcuff', type = 'player'},
        {condition = activeActions.canHandcuff, label = getLocalizedText('actions_put__hard_handcuffs'), value = 'hardHandcuff', type = 'player'},
        {condition = activeActions.canHandcuff, label = getLocalizedText('actions_start_dragging'), value = 'drag', type = 'player'},
        {condition = activeActions.canHandcuff, label = getLocalizedText('actions_put_in_car'), value = 'putInCar', type = 'player'},
        {condition = activeActions.canHandcuff, label = getLocalizedText('actions_take_from_car'), value = 'takeFromCar', type = 'vehicle'},
        {condition = activeActions.canLockpickCars, label = getLocalizedText('actions_lockpick_car'), value = 'lockpickCar', type = 'vehicle'},
        {condition = activeActions.canWashVehicles, label = getLocalizedText('actions:wash_vehicle'), value = 'washVehicle', type = 'vehicle'},
        {condition = activeActions.canRepairVehicles, label = getLocalizedText('actions:repair_vehicle'), value = 'repairVehicle', type = 'vehicle'},
        {condition = activeActions.canImpoundVehicles, label = getLocalizedText('actions:impound_vehicle'), value = 'impoundVehicle', type = 'vehicle'},
        {condition = activeActions.canCheckIdentity, label = getLocalizedText('actions:check_identity'), value = 'checkIdentity', type = 'player'},
        {condition = activeActions.canCheckVehicleOwner, label = getLocalizedText('actions:check_vehicle_owner'), value = 'checkVehicleOwner', type = 'vehicle'},
        {condition = activeActions.canCheckDrivingLicense or activeActions.canCheckWeaponLicense, label = getLocalizedText('actions:check_licenses'), value = 'checkLicenses', type = 'player'},
        {condition = activeActions.canHeal, label = getLocalizedText('actions:heal_small'), value = 'healSmall', type = 'player'},
        {condition = activeActions.canHeal, label = getLocalizedText('actions:heal_big'), value = 'healBig', type = 'player'},
        {condition = activeActions.canRevive, label = getLocalizedText('actions:revive'), value = 'revive', type = 'player'},
    ...
    for i=1, #actions do
        ...
        activeActions[action.value] = action.condition
    end
...
RegisterNetEvent(Utils.eventsPrefix .. ':actions:checkLicenses', function() 
    ...
    if(activeActions.canCheckDrivingLicense) then
        table.insert(elements, {label = getLocalizedText('actions:driving_license'), value = "driver"})
    end

    if(activeActions.canCheckWeaponLicense) then
