...
RegisterServerEvent("snipe-menu:server:changePlate", function(oldplate, newPlate)
    ...
    if src ~= 0 and onlineAdmins[src] then
        ...
        if Config.GarageTables[Config.Core].trunk_table then
            MySQL.Async.execute('UPDATE '..Config.GarageTables[Config.Core].trunk_table..' SET plate = ? WHERE plate = ?', {newPlate, oldplate})
        end
        if Config.GarageTables[Config.Core].glovebox_table then
            MySQL.Async.execute('UPDATE '..Config.GarageTables[Config.Core].glovebox_table..' SET plate = ? WHERE plate = ?', {newPlate, oldplate})
        end
    else
