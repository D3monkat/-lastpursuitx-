...
- In Jim-PizzaThis's `config.lua` set `JimConsumables` to true
- Restart/Ensure Jim-PizzaThis
...
### v1.7.5.1:
	- Fix incorrect variable being called, breaking the script
...
### v1.7.3:
	- Fix locale type breaking built-in consume event

### v1.7.2hotfix:
	- Fixed recipe changing using the wrong dough item
...
### v1.7.2:
	...
	- Fixed people getting stuck on dough machine
...
### v1.7.1
	...
	- Fix `lookEnt()` event
	- Remove debug prints when opening shop
	- Added more options to blip creation
	- Merged built-in eating events into one optimized event

### v1.7
	- Fix pizza box openning issue when using jim-consumables
	- Add Core export name change support in the config
	- Add support of ox_target
