...
if not Config.JimConsumables then
	CreateThread(function()
		...
		for k,v in pairs(drinks) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('frosty-lasttrain:client:Consume', source, item.name, "drink") end) end

		local food = { "cheesecake", "watermelon", "beandonut", "chocolate", "crisps", }
		for k,v in pairs(food) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('frosty-lasttrain:client:Consume', source, item.name, "food") end) end
	end)
else
	local foodTable = {
...
---Crafting
RegisterServerEvent('frosty-lasttrain:Crafting:GetItem', function(ItemMake, craftable)
	local src = source
	...
	if craftable then
		...
		for k, v in pairs(craftable[ItemMake]) do TriggerEvent("frosty-lasttrain:server:toggleItem", false, tostring(k), v, src) end
	end
	--This should give the item, while the rest removes the requirements
	TriggerEvent("frosty-lasttrain:server:toggleItem", true, ItemMake, amount, src)
end)
...
RegisterNetEvent('frosty-lasttrain:server:toggleItem', function(give, item, amount, newsrc)
...
local function CheckVersion()
	PerformHttpRequest('https://raw.githubusercontent.com/jimathy/UpdateVersions/master/frosty-lasttrain.txt', function(err, newestVersion, headers)
