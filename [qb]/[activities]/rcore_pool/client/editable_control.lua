...
function SetupAiming(fromCam, tableAddress)
    ...
    Citizen.CreateThread(function()
        while CurrentState == STATE_AIMING do
            ...
            RenderPreviewShotLine(currentTableCueBall.entity, cueBallPos, offsetPos)
...
function RenderPreviewShotLine(ballEntity, cueBallPos, offsetPos)
    if not Config.DrawShotPreviewLine then
        return
    end

    local visualShootVelocity = (cueBallPos.xyz - offsetPos.xyz)
    visualShootVelocity = visualShootVelocity/#visualShootVelocity*3
    local raiser = vector3(0.0, 0.0, 0.02)
    
    local visualRay = StartExpensiveSynchronousShapeTestLosProbe(
        cueBallPos + (visualShootVelocity/3*0.04) + raiser, 
        cueBallPos + visualShootVelocity + raiser, 
        1 + 16, 
        ballEntity, 
        4
    )

    local _, hit, visualEndCoords = GetShapeTestResultIncludingMaterial(visualRay)
    ReleaseScriptGuidFromEntity(entityHit)

    DrawLine(
        cueBallPos + raiser + vector3(0.0, 0.0, 0.005), 
        (hit == 1 and visualEndCoords) or (cueBallPos + visualShootVelocity + raiser) + vector3(0.0, 0.0, 0.005), 
        255, 255, 255, 255
    )
end
